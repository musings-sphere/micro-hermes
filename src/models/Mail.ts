...
export class Mail implements IMail {
	...
	public static create(props: IMail): Result<Mail> {
		const sourceAddressOrError = Guard.againstNullOrUndefined(
			props.sourceAddress,
			"sourceAddress"
		);
		const destinationAddressOrError = Guard.againstNullOrUndefined(
			props.destinationAddress,
			"destinationAddress"
		);
		const titleOrError = Guard.againstNullOrUndefined(
			props.messageTitle,
			"messageTitle"
		);
		const bodyOrError = Guard.againstNullOrUndefined(
			props.messageBody,
			"messageBody"
		);
